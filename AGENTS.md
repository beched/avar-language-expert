...
- **Dictionary lookup**: Use `avar.db` or `docs/avar_dictionary.md`, both generated from `docs/av-ru.dictionary.jsonl`
...
`avar.db` is generated by `build_dictionary.py` from `docs/av-ru.dictionary.jsonl`.
The JSONL was taken from [`avar-me/sources`](https://github.com/avar-me/sources), `data/av-ru.jsonl`.
The JSONL is the canonical source. Do not hand-edit the database.

Primary tables:
- `entries`: one row per JSONL entry, with full `entry_json`
- `forms`: inflected forms and gender forms mapped to `entry_id`
- `senses`: Russian sense text, comments, labels, and morphology metadata
- `examples`: Avar/Russian example pairs
- `see_also`: cross-reference graph
- `ru_index`: generated Russian → Avar lookup terms from senses and examples
- `legacy_avar`, `legacy_rus`: old SQLite-only fallback rows

Backward-compatible views:

```sql
-- Avar headword → Russian senses
SELECT word, translation FROM avar_rus WHERE word = 'абизе';

-- Russian lookup → Avar headwords
SELECT word, translation FROM rus_avar WHERE word = 'говорить';
```

Preferred form lookup:

```sql
SELECT e.headword, f.form, s.ru_text
FROM forms f
JOIN entries e ON e.id = f.entry_id
LEFT JOIN senses s ON s.entry_id = e.id AND s.sense_idx = 1
WHERE f.form_norm = 'абула';
```
...
**Dictionary:**
- Use SQLite form lookup first, then `avar_rus` / `rus_avar` compatibility views.
- Search `docs/avar_dictionary.md` for readable entries with examples.
...
For programmatic/SQL searches in `avar.db`, lookup keys are normalized into `*_norm` columns. Query those columns with palochka normalized to `Ӏ`.
...
# Structured dictionary (JSONL → SQLite + Markdown)
